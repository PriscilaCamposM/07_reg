...
Residuals looking suspicious. residuals are incresing
...
# Model fitting
mod3_quad <- lmer(yield_mgha ~ (1|rep) + sr_ksha + I(sr_ksha^2), data = reg_dfw)
...
Now, which one fits the data best? Which one should we chose for finding the optimum, or predicting new data?
...
summary(mod4_linp)
```
To know the numbers for start, look at the values from the intercept, slope you got from the model. 
## b) Model Assumptions
...
```
the geom smooth has some breaks because of bootsraps
